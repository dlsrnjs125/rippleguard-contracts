...
Event identity and domain identity are not interchangeable. `eventId` identifies one persisted Event, `causationId` points to the nearest persisted predecessor Event `eventId`, `correlationId` joins one business flow, and domain identifiers such as `agentRunId` identify business objects inside the payload. Expanding an existing v1.0.0 `causationId` contract to accept `agentRunId` is not a compatible clarification because it changes the meaning of the Event Envelope. If a future design needs Agent Run causation semantics, it requires a new contract version and scenario fixtures.

Phase 2 Governance validation event versions have distinct consumer meaning. `governance.agent-result.validated.v1` is retained for existing consumers and does not provide complete Snapshot and Model provenance for Audit Agent Run projection. `governance.agent-result.validated.v2` is a new major event type with required fixed provenance and `payload.requestEventId == causationId`. Consumers must reject unsupported major versions instead of interpreting v1 as v2 or filling missing provenance from side channels.

Published Phase 2 model manifests are Infra-materialized release evidence. Source-controlled templates use the separate `tabular-model-manifest-template.v1.0.0` contract and may contain an unresolved `runtimeImageDigest`, but `tabular-model-manifest.v1.0.0` remains the published manifest contract and must contain the exact OCI runtime image digest. Source commits, model artifact digests, all-zero/all-f placeholders and repeated-hex placeholders are not valid published runtime image digests.
