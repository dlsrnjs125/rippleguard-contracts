...
Published Phase 2 model manifests are Infra-materialized release evidence. A source-controlled `TEMPLATE` manifest may contain an unresolved `runtimeImageDigest`, but a `PUBLISHED` manifest must contain the exact OCI runtime image digest. Source commits, model artifact digests, all-zero/all-f placeholders, repeated-hex placeholders and explicit placeholder metadata are not valid published runtime image digests.
